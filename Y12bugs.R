...
load("C:/Users/ssbodda/Desktop/Saran/Research/Spring 2018/IRP/RSU/E2.RData")
Y <- cbind(Y1,Y2,Y3)
n <- 365
p <- 6
dat <- list(Y1=Y1,Y2=Y2,Y=Y,p=6,n=3)
...
mlr_inits <- function() {
  list(mu1 = mean(dat$Y[1,1:7],na.rm=T),mu2=rowMeans(Y[2:n,1:7],na.rm=T),rho=0)
}


samps <- bugs(data = dat, 
              inits = mlr_inits,
              parameters.to.save = c("mu1","mpx","mpxy"), 
              model.file = RE_model, 
              n.chains = 2, n.burnin=1e4, n.iter = 5e4,n.thin=10, DIC=F,debug = TRUE)
